...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class CountryModelFactory : ICountryModelFactory
    {
        ...
        protected virtual DistrictSearchModel PrepareDistrictSearchModel(DistrictSearchModel searchModel, StateProvince stateProvince)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
        ...
        protected virtual WardSearchModel PrepareWardSearchModel(WardSearchModel searchModel, District district)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            searchModel.DistrictId = district.Id;
            ...
            Country country, StateProvince state, bool excludeProperties = false)
        {
            ...
            if (state != null)
            {
                ...
                PrepareDistrictSearchModel(model.DistrictSearchModel, state);
            }
        ...
        public virtual DistrictListModel PrepareDistrictListModel(DistrictSearchModel searchModel, StateProvince stateProvince)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //get comments
            var districts = _stateProvinceService.GetDistrictsByStateProvinceId(stateProvince.Id, showHidden: true).ToPagedList(searchModel);

            //prepare list model
            var model = new DistrictListModel().PrepareToGrid(searchModel, districts, () =>
            {
                //fill in model values from the entity
                return districts.Select(state => state.ToModel<DistrictModel>());
            });

            return model;
        }
        public virtual DistrictModel PrepareDistrictModel(DistrictModel model,
            StateProvince state, District district)
        {
            if (district != null)
            {
                //fill in model values from the entity
                model = model ?? district.ToModel<DistrictModel>();
                PrepareWardSearchModel(model.WardSearchModel, district);
            }
        ...
        public virtual WardListModel PrepareWardListModel(WardSearchModel searchModel, District district)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //get comments
            var wards = _stateProvinceService.GetWardsByDistrictId(district.Id, showHidden: true).ToPagedList(searchModel);

            //prepare list model
            var model = new WardListModel().PrepareToGrid(searchModel, wards, () =>
            {
                //fill in model values from the entity
                return wards.Select(state => state.ToModel<WardModel>());
            });

            return model;
        }
        public virtual WardModel PrepareWardModel(WardModel model,
            District district, Ward ward)
        {
            if (ward != null)
            {
                //fill in model values from the entity
                model = model ?? ward.ToModel<WardModel>();
            }
