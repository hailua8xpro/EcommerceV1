...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        public virtual void PrepareBannerTypes(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));

            //prepare available product types
            var availableBannerTypeItems = BannerType.Home.ToSelectList(false);
            foreach (var bannerTypeItem in availableBannerTypeItems)
            {
                items.Add(bannerTypeItem);
            }
